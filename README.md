...
- [폴더 구조](#폴더-구조)
- [핵심 기능](#핵심-기능)
- [주요 API 엔드포인트 요약](#주요-api-엔드포인트-요약)
- [유저 플로우 요약 (Mermaid)](#유저-플로우-요약-mermaid)
- [라이브러리](#라이브러리)
- [인증 · 보안 설계](#인증--보안-설계)
- [운영 안정성 개선](#운영-안정성-개선)
- [실행 방법](#실행-방법)
...
- (5/16일 추가) MongoDB 연결 지연 처리로 인증 API 안정성 개선
...
![alt text](image.png)
 ...
 5/19일 로그인시 NextAuth API 라우트가 로드되는 시점에 MongoDB 연결이 즉시 시작되면서
 /api/auth/provider 요청까지 `MongoServerSelectionError`로 실패되는 지점의 코드를 수정했습니다.

- Auth 라우트 import 시 MongoDB 즉시 연결 수정
- 회원가입 API의 별도 MongoClient 생성 로직 제거
- 로그인/회원가입 등 DB가 필요한 시점에만 MongoDB 연결
