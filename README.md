...
- Next.js API Routes
- NextAuth (Credentials / Kakao / Naver)
- MongoDB 7
- TMDB API
- Sentry
...
- Presenter를 순수 UI 컴포넌트에 가깝게 유지해 변경 영향 범위를 줄이기 위함입니다.

**TanStack Query를 사용한 이유**

- 서버 상태 캐싱, 동기화, 재요청 제어를 일관되게 다루기 위함입니다.
- 로딩/에러/갱신 상태 처리를 표준화해 사용자 경험과 개발 생산성을 높이기 위함입니다.

**NextAuth를 사용한 이유**

- Credentials와 소셜 로그인(Kakao/Naver)을 하나의 인증 프레임워크에서 통합 관리하기 위함입니다.
- JWT 세션 전략과 콜백 기반 role 확장을 통해 인증/인가 구조를 명확히 드러내기 위함입니다.

**MongoDB를 사용한 이유**

- 예매, 좌석, 피드백, 즐겨찾기처럼 성격이 다른 데이터를 유연하게 모델링하기 위함입니다.
- Repository 계층과 결합해 조회/통계/마이그레이션 확장에 대응하기 위함입니다.

**Sentry를 사용한 이유**

- 클라이언트/서버/엣지 오류를 함께 수집해 문제 재현과 원인 파악을 빠르게 하기 위함입니다.
- 포트폴리오에서도 운영 관점의 관측성 구성을 포함했음을 보여주기 위함입니다.
...
John-Ahn 님의 강의를 참고하여 확장한 코드입니다. [📺](https://www.youtube.com/watch?v=RxPJeeEgdIs&t=1s)
